...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.viewfor = [NSMutableArray arrayWithObjects:@"", @"", @"", @"", nil];
    }
...
- (void)setCustomView:(UIView *)view for:(SVInfiniteScrollingState)state {
    ...
    if(state == SVInfiniteScrollingStateAll)
        [self.viewfor replaceObjectsInRange:NSMakeRange(0, 3) withObjectsFromArray:@[viewPlaceholder, viewPlaceholder, viewPlaceholder]];
    else
        [self.viewfor replaceObjectAtIndex:state withObject:viewPlaceholder];
...
- (void)setState:(SVInfiniteScrollingState)newState {
    ...
    id customView = [self.viewfor objectAtIndex:newState];
